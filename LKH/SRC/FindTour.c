...
GainType FindTour()
{
    ...
    for (Trial = 1; Trial <= MaxTrials; Trial++) {
        ...
        if (GetTime() - EntryTime < TimeLimit &&
            ...
            if (Dimension == DimensionSaved && fabs(Cost) >= fabs(OrdinalTourCost) &&
                fabs(BetterCost) > fabs(OrdinalTourCost) && !TSPTW_Makespan) {
        ...
        if (CurrentPenalty < BetterPenalty ||
            (CurrentPenalty == BetterPenalty && fabs(Cost) < fabs(BetterCost))) {
            if (TraceLevel >= 1) {
                printff("* %d: ", Trial);
            ...
            if ((BetterPenalty < BestPenalty ||
                 (BetterPenalty == BestPenalty && fabs(BetterCost) < fabs(BestCost))) &&
                SubproblemSize == 0)

            ...
            for (i = 0; i < PopulationSize; i++) {
                ...
                if (TraceLevel >= 1 &&
                    (CurrentPenalty < OldPenalty ||
                     (CurrentPenalty == OldPenalty && fabs(Cost) < fabs(OldCost)))) {
                    if (CurrentPenalty)
...
ProblemType != CVRPTW &&
        ...
        if (CurrentPenalty < BestPenalty ||
            (CurrentPenalty == BestPenalty && fabs(Cost) < fabs(BestCost))) {
        ...
        if (!Penalty ||
            ...
            if (CurrentPenalty == 0 && fabs(Cost) < fabs(Optimum))
                Optimum = Cost;
        } else if (CurrentPenalty < Optimum)
            Optimum = CurrentPenalty;
        if (fabs(Optimum) < fabs(OldOptimum)) {

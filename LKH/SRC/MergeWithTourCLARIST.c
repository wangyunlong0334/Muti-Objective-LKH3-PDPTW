...
GainType MergeWithTourCLARIST()
{
    ...
    BestCost = fabs(Cost1) <= fabs(Cost2) ? Cost1 : Cost2;
    ...
    if (NewPenalty2 < NewPenalty1 ||
        (NewPenalty2 == NewPenalty1 && fabs(Cost2) < fabs(BestCost))) {
        BestCost = Cost2;
    ...
    while ((N = N->Suc) != FirstNode);
    if (TraceLevel >= 2 && fabs(BestCost) < fabs(OldCost1) && fabs(BestCost) < fabs(OldCost2)) {
        printff("CLARIST: ");
        if (Penalty)
